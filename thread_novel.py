...
'Referer':'https://sou.xanbhx.com/search?siteid=qula&q=%E5%94%90%E9%97%A8%E9%AB%98%E6%89%8B%E5%9C%A8%E5%BC%82%E4%B8%96',
...
def get_txt(txt_id):
    ...
    try:
        ...
        # 获取小说所有章节信息
        all_page = soups.select('#wrapper .box_con #list dl dd a')
        # 获取小说总章页面数
        section_ct = len(all_page)
        #获取小说简介
        txt['intro']=soups.select('#wrapper .box_con #maininfo #intro')[0].text.strip()
        print("编号："+'{0:0>8}   '.format(txt['id'])+  "小说名：《"+txt['title']+"》  小说章节页数为："+str(section_ct)+"   开始下载。")
        #获取小说第一章页面地址
        # first_page = all_page[0]['href'].split('/')[3]
        first_page = all_page[12]['href']
        #设置现在下载小说章节页面
        txt_section=first_page
        #打开小说文件写入小说相关信息
        if os.path.exists('C:/usr/novel/'+'{0:0>8}-{1}.txt'.format(txt['id'],txt['title'])):
            print('小说：'+txt['title']+'已经存在，已停止下载')
            os._exit()
        if os.path.exists('C:/usr/novel/'+'{0:0>8}-{1}.txt.download'.format(txt['id'],txt['title'])):
            os.remove('C:/usr/novel/'+'{0:0>8}-{1}.txt.download'.format(txt['id'],txt['title']))
        fo = open('C:/usr/novel/'+'{0:0>8}-{1}.txt.download'.format(txt['id'],txt['title']), "ab+")
        fo.write((txt['title']+"\r\n").encode('UTF-8'))
        fo.write((txt['author'] + "\r\n").encode('UTF-8'))
        fo.write((txt['update'] + "\r\n").encode('UTF-8'))
        fo.write((txt['lately'] + "\r\n").encode('UTF-8'))
        ...
        while(1):
            try:
                ...
                sys.stdout.write('\r')
                sys.stdout.write(txt['title']+'   已下载'+ '{:.2%}'.format(count/(len(all_page)-12)))
                sys.stdout.flush()
                if count == all_page:
                    sys.stdout.write('\n')
                count +=1
                # 获取下一章地址
                txt_section = soup.select('#wrapper .content_read .bookname .bottem1 a')[2]['href']
        ...
        fo.close()
        os.rename('C:/usr/novel/'+'{0:0>8}-{1}.txt.download'.format(txt['id'],txt['title']), 'C:/usr/novel/'+'{0:0>8}-{1}.txt'.format(txt['id'],txt['title']))
    except:     #出现错误会将错误信息写入dowload.log文件，同时答应出来
        fo_err = open('C:/usr/novel/'+'dowload.log', "ab+")
        try:
            ...
            os.rename('C:/usr/novel/'+'{0:0>8}'.format(txt['id']) + '-' + txt['title'] + '.txt.download',
                  'C:/usr/novel/'+'{0:0>8}'.format(txt['id']) + '-' + txt['title'] + '.txt.error')
        except:
...
class thread(threading.Thread):
    def __init__(self, thread_id, novel_id, counter):
        threading.Thread.__init__(self)
        self.thread_id = thread_id
        self.novel_id = novel_id
        self.counter = counter
    def run(self):
        get_txt(self.novel_id)
if __name__ == '__main__':
    # number = [1983, 115563, 66788]
    number = [1983]
    for i in range(0, len(number)):
        try:
            threads = thread(i, number[i], i).start()
            print("即将下载编号为：", number[i], "的书")
        except:
            continue